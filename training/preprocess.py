...
class DataPreprocessor:
    ...
    def process(self, event: dict, augmentation: bool, inplace: bool = True) -> dict:
        ...
        # 保存处理前的波形数据
        #self._save_waveform(event["data"], title="Before Processing", identifier=timestamp, prefix="before")
        ...
        # 保存处理后的波形数据
        #self._save_waveform(event["data"], title="After Processing", identifier=timestamp, prefix="after")

        return event
    
    def _save_waveform(self, data: np.ndarray, title: str, identifier: str, prefix: str):
        """Save waveform data to a file.

        Args:
            data (np.ndarray): Waveform data.
            title (str): Plot title.
            identifier (str): Unique identifier for the file.
        ...
        plt.tight_layout()
        filename = f"{prefix}_processing_{identifier}.png"
        plt.savefig(filename)
        plt.close()
        print(f"Saved {title} waveform to {filename}")
